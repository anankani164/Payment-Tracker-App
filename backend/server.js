// backend/server.js
// Patched: adds GET /api/clients/:id/statement (auth) while keeping your existing endpoints.
// Also includes a simple /api/health for convenience.
...
// Create client (auth required). Wrap in role check if needed.
...
app.get('/api/invoices', (req,res)=>{
  ...
  const rows = query(db, `
    SELECT
      ...
      CASE
        WHEN i.status <> 'paid' AND i.due_date IS NOT NULL AND datetime(i.due_date) < datetime('now')
...
app.post('/api/invoices', auth, (req,res)=>{
  ...
  run(db, `INSERT INTO invoices(client_id,title,description,total,due_date,created_at,created_by)
           VALUES(?,?,?,?,?,?,?)`,
...
app.post('/api/payments', auth, (req,res)=>{
  ...
  const createdBy = req.user?.id || null;
  run(db, `INSERT INTO payments(invoice_id, amount, percent, method, note, created_at, created_by)
           VALUES(?,?,?,?,?,?,?)`,
...
// Protected by `auth`. Accepts only numeric client ids.
app.get('/api/clients/:id/statement', auth, (req, res) => {
  ...
  const client = query(db, `SELECT id, name, email, phone FROM clients WHERE id=?`, [clientId])[0];
...
app.listen(PORT, ()=> console.log('Backend (auth+roles+created_by+clients+statement) running on :' + PORT));
